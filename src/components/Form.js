...
const Form = ({setStatus}) => {
  ...
  const Placebid = async (e) => {
    ...
    if ( !pincode || !Price || !maxunit || !users) {
    ...
    if(Price  <  0 || Price == 0  ){
     ...
     if((Price * maxunit) / 80  > 10000 ){
      message.error("please lower the price or the maxunit")
      return;
     }

    try {
      
      const data = {  pincode, Price, maxunit, users , userid  };

      

      const order = await axios.post(
        "https://3.6.225.138/api/mak/market",
        data
      );
