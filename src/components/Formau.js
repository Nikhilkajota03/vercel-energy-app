...
const Formau = ({setStatus}) => {
  ...
  const [users , setusers] = useState(user)
   ...
   const Placebid = async (e)=>{
      ...
      if (!pincode || !minPrice|| !users || !maxunit) {
      ...
      if(minPrice  <  0 || minPrice == 0  ){
        ...
        try {
            ...
            const data  = { pincode , minPrice , maxunit , users , userid };
            

            const order = await axios.post("https://3.6.225.138/api/auct/auctionord" , data);
